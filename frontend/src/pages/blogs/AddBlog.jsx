...
import { createBlog } from '../../services/dataService';
...
export default function AddBlog() {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      await createBlog({ ...data, content, image: URL.createObjectURL(image), publishDate: new Date().toISOString().split('T')[0], author: 'Admin' });
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: 'Blogs', href: '/blogs' }, { label: 'Add Blog' }]} />
      <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Add New Blog Post</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            <div className="rounded-xl bg-white p-6 shadow-subtle dark:bg-slate-800 space-y-4">
              ...
              <FormSelect label="Status" name="status" register={register} errors={errors} options={[{ label: 'Published', value: 'Published' }, { label: 'Draft', value: 'Draft' }]} />
            </div>
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => navigate('/blogs')} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
            Cancel
          </button>
